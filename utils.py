...
def parse_amount(s):
    """Safely convert mixed text like '₹1,250.50' into float(1250.5)."""
    try:
        if pd.isna(s):
            return 0.0
...
# ---------------------------------------------------------------------
# Main Function: Fuzzy Match Orders ↔ Payments
# ---------------------------------------------------------------------
...
def fuzzy_match_orders_to_payments(
    ...
    """
    Match orders to payments using:
      1) Direct OrderID match
      2) Amount proximity (within tolerance)
      3) Fuzzy ID similarity
    Returns merged dataframe with match_type flag.
    """
    ...
    # -----------------------------------------------------------------
    # Always guarantee that join columns exist BEFORE merging
    # -----------------------------------------------------------------
    ...
    # Ensure numeric columns exist and are parsed
    ...
    # -----------------------------------------------------------------
    # Safe merge (no KeyError possible)
    # -----------------------------------------------------------------
    merged = pd.merge(
        o,
        p,
        left_on=order_id_col,
        right_on=payment_id_col,
        how="left",
        indicator=True,
    ...
    merged["match_type"] = merged["_merge"].map(
        {"both": "direct_id", "left_only": None, "right_only": "payment_only"}
    )

    # -----------------------------------------------------------------
    # Step 2: Amount-based matching (greedy)
    # -----------------------------------------------------------------
    ...
    for idx, row in unmatched_orders.iterrows():
        ...
        cand = payments_pool[
            (payments_pool["__used__"] == False)
            & (abs(payments_pool[payment_amount_col] - amt) <= amount_tolerance)
        ]
        if not cand.empty:
...
# ---------------------------------------------------------------------
# Function: Bank-statement / batch reconciliation
# ---------------------------------------------------------------------

def group_orders_by_batch_and_match(
    orders_df,
    bank_stmt_df,
    order_amount_col="__order_amount__",
    date_col_orders=None,
    date_col_bank=None,
    days_window=2,
):
    """
    ...
    o[order_amount_col] = (
        o[order_amount_col].apply(parse_amount)
        if order_amount_col in o.columns
        else 0.0
    )
